...
int main(int argc, char *argv[])
{
	...
	kg = (struct kgarten_struct *)safe_malloc(sizeof(*kg));
	kg->vt = kg->vc = 0;
	kg->ratio = ratio;
	 ...
	 * Create threads
	 */
	thr = (struct thread_info_struct *)safe_malloc(thrcnt * sizeof(*thr));

	for (i = 0; i < thrcnt; i++) {
		/* Initialize per-thread structure */
		thr[i].kg = kg;				// address
		thr[i].thrid = i; 			// relative id
		thr[i].thrcnt = thrcnt;		// total_threads
		thr[i].is_child = (i < chldcnt); // child_ids = {0, .., c - 1}
										 // teacher_ids = {c, .., total_threads - 1}
		thr[i].rseed = rand();

		/* Spawn new thread
		 *  thread_start_fn (struct thread_info_struct *)
		 */

...
void process_cmdline(char *cmdline, int wfd, int rfd)
{
	...
	/* Print Tasks */
	if (strcmp(cmdline, "p") == 0 || strcmp(cmdline, "P") == 0) { // REQ_PRINT_TASKS
	...
	if ((cmdline[0] == 'k' || 
		cmdline[0] == 'K') &&
	    cmdline[1] == ' ') { // REQ_KILL_TASK	
		rq.request_no = REQ_KILL_TASK;
		rq.task_arg = atoi(&cmdline[2]); // get process ID
		issue_request(wfd, rfd, &rq);
		return;
	}
	...
	if ((cmdline[0] == 'e' || 
		cmdline[0] == 'E') && 
		cmdline[1] == ' ') { // REQ_EXEC_TASK
		rq.request_no = REQ_EXEC_TASK;
		strncpy(rq.exec_task_arg, &cmdline[2], EXEC_TASK_NAME_SZ); // parse name of executable
	...
	if ((cmdline[0] == 'h' ||
	    cmdline[0] == 'H') && 
	    cmdline[1] == ' ') { // REQ_HIGH_TASK
		rq.request_no = REQ_HIGH_TASK;
		rq.task_arg = atoi(&cmdline[2]); // parse process ID
		issue_request(wfd, rfd, &rq);
		return;
	}
